            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-2">
                ...
                <div class="mt-6 p-2 bg-slate-100">
                        ...
                        @if ($user->roles)
                            @foreach ($user->roles as $user_role)
                                <form class="px-4 py-2 bg-red-500 hover:bg-red-700 text-white rounded-md" method="POST"
                                    action="{{ route('superadmin.users.roles.remove', [$user->id, $user_role->id]) }}"
                    ...
                    <div class="max-w-xl mt-6">
                        <form method="POST" action="{{ route('superadmin.users.roles', $user->id) }}">
                            @csrf
                ...
                <div class="mt-6 p-2 bg-slate-100">
                    ...
                    <div class="flex space-x-2 mt-4 p-2">
                        @if ($user->permissions)
                            @foreach ($user->permissions as $user_permission)
                                <form class="px-4 py-2 bg-red-500 hover:bg-red-700 text-white rounded-md" method="POST"
                                    action="{{ route('superadmin.users.permissions.revoke', [$user->id, $user_permission->id]) }}"
                                    onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div class="max-w-xl mt-6">
                        <form method="POST" action="{{ route('superadmin.users.permissions', $user->id) }}">
                            @csrf
                            <div class="sm:col-span-6">
